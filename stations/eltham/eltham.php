...
<?php
	...
	if ($percentage > 50)
	{
		echo "<span class='stationTitleOpen'>ELTHAM</span>
				<button class='stationTitleButtonOpen'>Open</button>";
	}
	else
	{
		echo "<span class='stationTitleClosed'>ELTHAM</span>
...
atLocation = 0;
	...
	function addYes()
	{
		...
		{
			//Delete noCookie
			document.cookie = "name=noCookie; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/stations/eltham";
			...
			yesCookie = document.cookie = "name=yesCookie; expires=" + cookieTime + "; path=/stations/eltham";

			//Vote yes to the DB
			req = new XMLHttpRequest();
			req.open("GET", "elthamYes.php", true);
			req.send();
			location.reload();
		}
		else if ((document.cookie.indexOf('yesCookie') == -1) && (document.cookie.indexOf('noCookie') == -1) && (atLocation == 1))
	...
	function addNo()
	{
		if (document.cookie.indexOf('noCookie') != -1)
			...
			alert("You have already said no. Why don't you eat a pancake and wait a while?")
                ...
                {
                        //Delete yesCookie
                        document.cookie = "name=yesCookie; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/stations/eltham";
                        ...
                        var expiryMonth = date1.toLocaleString('default', {month: 'short'})
                        var cookieTime = date1.getDate() + ' ' + expiryMonth + ' ' + date1.getFullYear() + ' ' + expiryHour + ":" + date1.getMinutes() + ":00 UTC";
                        noCookie = document.cookie = "name=noCookie; expires=" + cookieTime + "; path=/stations/eltham";

                        //Vote yes to the DB
                        req = new XMLHttpRequest();
                        req.open("GET", "elthamNo.php", true);
                        req.send();
                        location.reload();
		}
		else if ((document.cookie.indexOf('noCookie') == -1) && (document.cookie.indexOf('yesCookie') == -1) && (atLocation == 1))
